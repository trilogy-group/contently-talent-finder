...
export const ContentAnalyzer = ({ initialContent = "", documentType, brandVoice }: ContentAnalyzerProps) => {
  // Use the initial content if provided, otherwise use an empty string (NOT the demo text)
  ...
  // Use demo text for suggestions if content is empty
  const textToAnalyze = content || demoText;
  const suggestions = content ? [] : sampleSuggestions; // Only show suggestions for demo content

  const renderHighlightedContent = () => {
    let lastIndex = 0;
    const elements: JSX.Element[] = [];
    const currentSuggestions = content ? [] : sampleSuggestions; // Only show suggestions for demo content
